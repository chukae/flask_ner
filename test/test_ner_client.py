...
class TestNerClient(unittest.TestCase):
    ...
    def test_get_ents_returns_dictionary_given_nonempty_string_causes_empty_spacy_doc_ents(self):
        ...
        model.returns_doc_ents([])
        ner = NamedEntityClient(model)
        ents = ner.get_ents("Lagos is a place in Portugal")
        self.assertIsInstance(ents, dict)
    ...
    def test_get_ents_given_spacy_LOC_is_returned_serializes_to_Loc(self):
        model = NerModelTestDouble('eng')
        doc_ents = [{'text': 'the sky', 'label_': 'LOC'}]
        model.returns_doc_ents(doc_ents)
        ner = NamedEntityClient(model)
        result = ner.get_ents('xxxxxxx')
        expected_result = {'ents': [{'ent': 'the sky', 'label': 'Location'}], 'html': ""}
        self.assertListEqual(result['ents'], expected_result['ents'])
